...
public class TentativeConnexion extends Tache {
	...
	public static void testLocal(boolean serveur, int perso) {
		...
		try {
			...
			t.addAvancementListener(new AvancementListener() {
				@Override public void setAvancement(int pourcentage) {
					if(pourcentage == 100) try {
						ChargementPartie c = new ChargementPartie(t);
						c.addFinChargementListenerListener(new FinChargementListener() {
							@Override public void finChargement() {try {
