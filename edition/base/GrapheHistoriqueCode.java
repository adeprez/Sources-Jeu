...
public class GrapheHistoriqueCode extends Ecran implements ActionListener, MouseMotionListener {
    ...
    private final JCheckBox lignes, classes;
	...
	g.setColor(Color.BLACK);
	long j = (max - min)/1000/60/60/24;
	if(!codes.isEmpty())
	    g.drawString(codes.size() + " fois, " + j + " jours (" + codes.get(codes.size() - 1).lignes/j + " lignes/jour)", 10, 20);
