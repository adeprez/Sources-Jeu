...
public class CreateurObjets extends Ecran implements ActionListener, ChangeFormeListener, ChangeEtatListener {
	...
	case BLOC:
	    ...
	    break;
	case ECHELLE:
	    objet = new Echelle(map, SpriteObjets.getInstance(), image.getIdFond(), forme, image.getIdImage(), resistance, degats, decoupe);
	    break;
	case CORDE:
	    objet = new Corde(map, SpriteObjets.getInstance(), image.getIdFond(), new FormeVide(), image.getIdImage());
	    break;
	default:
